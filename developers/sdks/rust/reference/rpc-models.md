# Rust SDK RPC Result Models

RPC wrappers decode JSON responses into public structs. Large numeric chain
quantities are modeled as `String` when the corresponding result fields are
declared as strings in the SDK.

The SDK does not hide the node's data model. Result structs mostly mirror the
...
| ------ | ------- | ------ | -------------------- |
| `convert_decimals(amount, decimals)` | Formats a base-unit integer string with a decimal point. | `amount`: base-unit text. `decimals`: number of fractional digits. | Returns `String`. It trims leading zeros before placing the decimal point, trims trailing fractional zeroes, preserves a leading `-`, and suppresses negative zero. It does not fetch token metadata or validate that `amount` belongs to a token with matching decimals. |
...
| `AccountResult::token_balance(symbol)` | Finds an existing balance row without mutating the account. | `symbol`: exact token symbol. | Returns `Option<&BalanceResult>`. |
| `AccountResult::get_token_balance(symbol, decimals)` | Finds or creates a balance row for a token symbol in an already-loaded account response. | `symbol`: exact token symbol. `decimals`: decimals for the inserted zero row when the symbol is absent. | Returns `&BalanceResult`. If no matching row exists, appends `BalanceResult { chain: "main", amount: "0", symbol, decimals, ids: [] }` to `balances` and returns it. |
| `TransactionResult::state_is_success()` | Classifies transaction state. | None. | Returns `tx_state_is_success(&self.state)`. |
| `TransactionResult::state_is_fault()` | Classifies fault/break transaction state. | None. | Returns `tx_state_is_fault(&self.state)`. |
...
| `TokenResult::is_non_fungible()` | Checks token `NonFungible` flag. | None. | Returns `has_flag("NonFungible")`. |
...
| `ScriptResult::decode_result()` | Decodes the single VM result hex string. | None. | Returns `Result<VMObject>`. Fails on invalid hex or VM object decoding errors. |
| `ScriptResult::decode_results(index)` | Decodes one indexed VM result hex string. | `index`: index into `results`. | Returns `Result<VMObject>`. Fails with `PhantasmaError::Rpc { code: None, ... }` if the index is out of range; otherwise fails on invalid hex or VM object decoding errors. |
...
Use `PaginatedResult<T>` for older page/page-size RPCs such as auctions and
address transaction history. Use `CursorPaginatedResult<T>` for large token/NFT
inventory queries where the node returns an opaque cursor.
...
token is not present in the account response.
...
`state_is_success()` and `state_is_fault()` to classify finalized execution
state. `ScriptResult` is returned by read-only script invocation. Use `result`
for the single-result path and `results` for indexed VM results.
...
| `VmVariableSchemaResult` | `type_name`, `schema` |
...
| Model | Fields |
| ----- | ------ |
| `AbiParameterResult` | `name`, `type_name` |
| `AbiMethodResult` | `name`, `return_type`, `parameters` |
| `AbiEventResult` | `name`, `value`, `return_type`, `description` |
...
| `ValidatorResult` | `address`, `type_name` |
...
| `VersionResult` | Alias of `BuildInfoResult` |
