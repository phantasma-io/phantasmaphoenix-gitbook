...
| `with_max_response_bytes(max_response_bytes)` | Replaces the `ReqwestTransport` response body limit. The default is `DEFAULT_MAX_RPC_RESPONSE_BYTES`, currently 16 MiB. | Returns `Self`. Errors happen when a later response exceeds the configured limit. | A caller needs endpoint-specific response-size policy. |
...
| `call_value(method, params)` | Sends a raw JSON-RPC call with a generated id and returns the raw JSON `result`. | Returns `serde_json::Value`. Errors if transport fails, the body is not a valid JSON-RPC response, the id does not match, the response contains `error`, or `result` is missing. | A node method is needed before the SDK has a typed wrapper. |
| `call_value_with_raw(method, params)` | Sends a raw call and keeps response metadata together with the raw `result`. | Returns `RpcCallResult<Value>` with endpoint, method, HTTP status, raw envelope, raw result, and canonical JSON byte counts. | Diagnostics or tooling need the decoded result and the original response envelope. |
...
| `call_with_raw<R>(method, params)` | Calls `call_value_with_raw(...)` and deserializes the raw result into `R` while preserving metadata. | Returns `RpcCallResult<R>`. Converts deserialization failures into `PhantasmaError::Rpc`. | Wrapper code needs typed decoding plus response metadata. |
| `parse_json_rpc_response(status, body)` | Validates a JSON-RPC response object for the initial request id and extracts `result`. | Returns raw `Value` or `PhantasmaError::Rpc`. | Custom transports reuse SDK validation with the initial id. |
| `parse_json_rpc_response_for_request(status, body, expected_request_id)` | Validates a JSON-RPC response object against a specific request id. It treats JSON-RPC `error` objects as failures and rejects missing or mismatched ids. | Returns raw `Value` or `PhantasmaError::Rpc`. | Custom transports track request ids outside `PhantasmaRpc`. |
