...
Carbon is the native asset runtime for tokens and NFTs on Phantasma Phoenix. Asset operations (create, mint, transfer, burn) are built into the chain, so standard token and NFT flows do not require custom smart contracts or VM execution. Metadata is schema-validated by the chain. See the full overview: [Carbon: Native Assets on Phantasma Phoenix](/about/carbon.md).
